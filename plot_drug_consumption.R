...
#
# Loops over every drug marked cancer_drug == "yes" in the enriched CSV,
# plots Medicare Part D consumption (Total Dosage Units), fits a linear
# trend on the pre-2021 period, and projects 95% prediction intervals
# onto 2021-2023, annotating % deviation from the trend.
###############################################################################
...
# ── Configuration ──────────────────────────────────────────────────────────────
enriched_csv   <- "medicare_partd_dosage_units_2023_llm_enriched.csv"
consumption_csv <- "medicare_partd_dosage_units_2012_2023.csv"
output_dir     <- "display"
...
# ── Load enriched CSV & extract cancer drugs ──────────────────────────────────
enriched <- read.csv(enriched_csv, stringsAsFactors = FALSE)
cancer_drugs <- enriched |>
  filter(tolower(trimws(cancer_drug)) == "yes") |>
  select(brand_name, generic_name, notes)

cat("Found", nrow(cancer_drugs), "cancer drugs to plot:\n")
print(cancer_drugs |> select(brand_name, generic_name))
cat("\n")

# ── Load consumption data ─────────────────────────────────────────────────────
raw <- read.csv(consumption_csv, stringsAsFactors = FALSE)
...
# ── Loop over cancer drugs ────────────────────────────────────────────────────
for (i in seq_len(nrow(cancer_drugs))) {

  drug_brand   <- trimws(cancer_drugs$brand_name[i])
  drug_generic <- trimws(cancer_drugs$generic_name[i])
  drug_notes   <- trimws(cancer_drugs$notes[i])

  # Build a safe filename from the brand name
  safe_name   <- gsub("[^A-Za-z0-9_-]", "_", drug_brand)
  safe_name   <- gsub("_+", "_", safe_name)
  safe_name   <- tolower(trimws(safe_name, whitespace = "_"))
  output_file <- file.path(output_dir, paste0(safe_name, "_consumption.png"))

  cat(sprintf("[%d/%d] %s (%s) → %s\n",
              i, nrow(cancer_drugs), drug_brand, drug_generic, output_file))
  ...
  if (nrow(df) == 0) {
    cat("  ⚠ No consumption data found — skipping\n")
    next
  }
  ...
  if (nrow(train) < 3) {
    cat("  ⚠ Fewer than 3 training years — skipping\n")
    next
  }

  fit <- lm(total_dosage_units ~ year, data = train)

  # ── Predictions + 95% PI for ALL years ──────────────────────────────────
  ...
  p <- ggplot(plot_df, aes(x = year)) +
    ...
    # ── Labels ──
    labs(
      title    = paste0(drug_brand, " (", drug_generic, ")"),
      ...
      caption  = paste0(
        ...
        "slope = ", ifelse(coef(fit)["year"] >= 0, "+", ""),
        format(round(coef(fit)["year"]), big.mark = ","),
        " units/year  |  ", drug_notes
      )
    ) +
